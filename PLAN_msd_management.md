...
On approval this file is copied verbatim to `PLAN_msd_management.md` in the project root
(the harness plan path is outside the repo; the root copy is the one future sessions read).
...
as-built item), `.env.example`, `CLAUDE.md` (State, Config tiers, storage section, drift list
